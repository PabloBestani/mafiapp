import type { PlayerKind, PlayerProfile } from "../domain/types";
...
export class SQLitePlayerRepository implements PlayerRepository {
  ...
  async savePlayer(player: PlayerProfile): Promise<void> {
    await this.database.run(
      `INSERT INTO players (id, name, kind, created_at, updated_at, deleted_at)
       VALUES (?, ?, ?, ?, ?, NULL)
       ON CONFLICT(id) DO UPDATE SET
         ...
         deleted_at = NULL;`,
      [player.id, player.name, player.kind, player.createdAt, player.updatedAt]
    );
  }
